...
import { Code, HStack, Text, Button, Alert, AlertIcon, Box, AlertTitle, AlertDescription, List, ListItem } from "@chakra-ui/react";
import { isErrorWellFormed, isConsentRequiredError, isAuthorizationRequirementsError } from "@globus/sdk/cjs/lib/core/errors";

import { useOAuthContext } from "../globus-api/GlobusOAuthProvider";

// NOTE: uses the Globus API for error handling:
//    https://globus.github.io/globus-sdk-javascript/modules/_globus_sdk.Errors.html

/*
 * Error handler for the file browser
 * Handles errors that were returned from lsEndpoint by printing the error message
*/

export default function FileBrowserError({ error }) {
  const auth = useOAuthContext();

  const isWellFormed = isErrorWellFormed(error);

  // App requires the user's consent to access the requested resources
  ...
  // Credentials are invalid, need to refresh
